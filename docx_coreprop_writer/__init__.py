...
def overwrite_meta(meta_file, filename, meta_ext):
    ...
    if meta.created is not None:
        ...
        """
        doc.core_properties.created = datetime.datetime.strptime(meta.created, "%Y-%m-%d")  # YYYY-MM-DD
    ...
    if meta.last_printed is not None:
        ...
        """
        doc.core_properties.last_printed = datetime.datetime.strptime(meta.last_printed, "%Y-%m-%d")
    if meta.modified is not None:
        """modified (datetime)
        Date on which the resource was changed. (Dublin Core)
        """
        doc.core_properties.modified = datetime.datetime.strptime(meta.modified, "%Y-%m-%d")
